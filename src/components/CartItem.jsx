...
const CartItem = ({ item, onIncrease, onDecrease }) => (
  <div className="flex items-center justify-between bg-gray-300 rounded-md p-2 shadow-sm">
    ...
    <div className="flex items-center space-x-2">
      <button onClick={() => onDecrease(item)} className="px-2 bg-gray-200 rounded hover:bg-gray-300">-</button>
      <span>{item.quantity}</span>
      <button onClick={() => onIncrease(item)} className="px-2 bg-gray-200 rounded hover:bg-gray-300">+</button>
    </div>
